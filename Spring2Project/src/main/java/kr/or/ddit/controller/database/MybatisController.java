...
public class MybatisController {
	 ...
	 *  		- 여기까지 확인
	 *  
	 *  		- 게시판 상세 화면 컨트롤러 메소드 만들기(curdRead:get)
	 ...
	 * 		
	 * 		- '_'가 포함되어 있는 데이터베이스 컬럼명을 Camel기법 셋팅으로 인해서 board_no가 boardNo로 처리된다.
	 * 
	 * 		1) 마이바티스 설정
	 * 
	 * 			1-1) mybatisAlias.xml 설정
	 * 				<settings>
	 * 					<setting name="mapUnderscoreToCalmelCase" value="true"/> 설정 추가
	 * 				</settings>
	 * 
	 ...
	 * 5. 기본키 취득
	 * 
	 * 		- 마이바티스는 userGenerateKeys 속성을 이용하여 insert할 때 데이터 베이스 측에서 채번된 기본키를 취득할 수 있다.
	 * 		1) 마이바티스 설정
	 * 			1-1) 매핑 파일 수정(boardMapper_SQL.xml)
	 * 			- create 부분의 속성 추가
	 * 				> userGenerateKeys="true" (insert 태그 내 속성으로 추가)
	 ...
	 * 			생성된 boardNo를 sql쿼리가 담겨있는 insert 태그까지 넘어올때, 넘겨주고 있는 파라미터(board)의 property인
	 * 			boardNo에 셋팅된다.
	 * 			셋팅된 boardNo를 아래에서 insert 시 등록 값으로 사용하고 board라는 자바빈즈 객체에 담겨 최종 컨트롤러까지
	 * 			넘어가서 넘겨받은 boardNo를 이용한 상세보기 페이지로의 이동이 가능하다.
	 * 
	 * 			*** seq_board.nextval 대신 현재의 seq번호를 가져오기 위해 currval를 사용하게 되는 경우가 있습니다.
	 * 			이때 주의사항이 있습니다!
	 * 
	 * 				- select_seq_board.currval from dual
	 * 				> 위 select 쿼리를 사용 시, currval를 사용하는데 있어서 사용 불가에 대한 에러가 발생할 수 있다.
	 * 					currval를 사용할 때는 select seq_board.nextval from dual로 먼저 최조 실행 후,
	 * 					select seq_board.currval from dual로 사용하면 에러가 없음!!!!!!!!!
	 * 					같은 세션 내에서의 실행이 이뤄지지 않았기 때문에 currvall로 데이터를 가져오는데 에러가 발생한다.
	 * 
	 * 				# 그럼에도 나는 가져와야겠다! 다 필요없고 그냥 줘!
	 ...
	 *
	 *				- 완료 후 성공 페이지로 이동하지만, 넘겨 받은 boardNo 즉, 게시글을 등록 후 생성된 최신 글번호(boardNo)가
	 *					있으므로 글번호를 이용하여 상세보기 페이지로의 이동이 가능하다.
	 *					성공 후 결과페이지(success)로 넘어간 후 목록으로 전환해도 되고, 최신 게시글 번호를 이용한 상세보기 페이지로의
	 *					이동으로 프로세스를 전환해도 무방! (자유롭게)
	 *
	 *
	 * 			
	 * 	6. 동적 SQL
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 회원 등록 기능 컨트롤러 메소드 만들기(crud MemberRegister:post)
