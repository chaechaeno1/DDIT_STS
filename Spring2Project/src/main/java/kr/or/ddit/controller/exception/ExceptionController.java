 ...
 *	1. 예외처리
 *
 *		일반적으로 프로그램이 처리되는 동안 특정한 문제가 일어났을 떄, 처리를 중단하고 다른 처리를 하는 것을 예외처리라고 한다.
 * 		웹 컨테이너는 기본적으로 예외처리를 하여 기본 에러 페이지를 표시해준다.
 * 		하지만, 페이지에 애플리케이션 서버의 내부 정보가 일반 사용자들에게 노출되어 프레임워크의 보안 취약점을 노린 공격을
 * 		받을 수 있다. 이런 점을 고려하여 최대한 사용자가 직접 예외를 처리하여 사용자가 정의한 에러 페이지를 표시하게 해야한다.
 * 
 * 
 * 		1-1) 예외 종류
 * 		- 스프링 프레임워크 예외
 ...
 * 		- C) 존재하지 않는 게시물 조회할 떄, 사용자가 정의한 예외 발생
 ...
 * 
 * 			- 존재하지 않는 게시물을 조회할 떄 사용자가 정의한 예외 발생
 * 				> Http Status 500
 * 				> [예외 상황 만들 위치]
 * 					kr.or.ddit.service.impl.BoardServiceImpl read() 수정
 * 					new BoardRecordNoFoundException 예외 발생
 * 
 ...
 * 
 * 	
 * 	4. 예외 타입 사용한 에러페이지 설정
 * 
 * 		- 웹 컨테이너 설정 파일(web.xml)의 <exception-type> 요소에 예외 타입을 설정하고 <location> 요소에 이동 대상
 * 		페이지를 지정한다.
 * 
 * 			예외 처리 방법
 * 			- 웹 컨테이너 설정(web.xml)
 ...
 * 		- 처리할 수 없는 예외
 * 			> 수정 화면 생성할 때 뷰 파일에서 예외 발생
 * 				> ServletException을 상속받는 JasperException 에러 발생이므로 Exception 처리를 타지않음
 * 			> 존재하지 않는 페이지 URL 요청 시 예외 발생
 * 				> 404 종류에 해당하는 에러기 때문에 예외 타지 않음
 * 
 * 	
 * 
 * 	7. 예외 정보 출력
 ...
 * 			예외 처리 방법
 * 			- 웹 컨테이너 설정
 * 				> 404에러를 처리할 수 있도록 DispathcherServlet의 throwExceptionIFNoHandlerFound 속성을 true로 설정
 * 
 * 			처리할 수 있는 예외
 * 			> 존재하지 않는 페이지 UTL 요청 시 예외 발생
 * 
 * 			처리할 수 없는 예외
 * 
 * 
