...
public class AjaxMemberController {
	...
	public ResponseEntity<String> ajaxRegister04(String userId, String password){
		...
		// 요청 본문에서 데이터를 가져오려면 @RequestBody 어노테이션을 꼭 붙여주어야 한다.
		// 그래서, 단일 데이터를 컨트롤러 메서드에서 값으로 받기 위해서는 기본 타입 변수를 사용하지 않고
		// 컬렉션 Map을 활용하도록 합니다 (이때에도 @RequestBody는 필수겠죠?)
	...
	// 헤더 영역의 값은 꺼낼 수 있음. (url : "/ajax/register05?userId=" + userId, )
	@ResponseBody
	...
	public ResponseEntity<String> ajaxRegister07(
		...
		// 비동기 처리시, List 컬렉션으로 데이터를 받을 때에는 @RequestBody라는 어노테이션을 이용하여 바인딩 한다.
		// 동기 처리시에는 컨트롤러 메서드 내에서 List 타입으로 값을 바인딩 할 수 없지만, 객체 내 존재하는 List타입으로는 데이터를 바인딩 할 수 있다.
		// 동기와 비동기처리시의 차이점을 꼭 기억합시다!!!!!!
	...
	public ResponseEntity<String> ajaxRegister08(
		...
		Address address = member.getAddress(); //중첩된 객체 선언
		if(address != null) {
