...
public class JSPFormtagController {
	 ...
	 * 				
	 * 				path : 폼 항목에 바인딩 되는 폼 객체의 프로퍼티를 지정한다.
	 * 				disabled : 폼 항목을 비활성화할지 여부 지정. 기본값은 false
	 * 				readonly : 폼 항목을 읽기 전용으로 만들지 여부를 지정. 기본값은 false
	 * 
	 * 				※ input 요소에 disabled(수정이 안되면서 서버로안넘어감)와 readonly(수정이안되면서 데이터넘어감)
	 * 
	 * 
	 * ** 유지보수 업무할 때 스프링 Form 태그 종종 이용됨
	 * 
	 * 
	 */
