...
public class JSPHomeController {
	 ...
	 * 			- JSP가 제공하는 지시자에는 page 지시자, taglib지시자, include 지시자로 세가지가 있다.
	 * 
	 * 				1) page 지시자
	 ...
	 * 			- 이 문제를 해결하는 한 가지 방법은 커스텀태그를 사용하는 것이다.
	 * 			- 커스텀태그르 사용하면 뷰를 표시하기 위한 로직을 공통화하거나, 표현하기 복잡한 로직을 캡슐화할 수 있어서 
	 * 			- JSP의 구현 코드를 간결하게 만들 수 있다. 그리고, 커스텀 태글르 모아 놓은 것을 커스텀 태그 라이브러리라고 한다.
	 * 
	 * 			# 대표적인 태그 라이브러리
	 ...
	 * 				# 논리 연산자
	 * 				─────────────────────────────────────────────────────
	 * 				&&(and)		│	두 피연산자 모두 true이면 bool true를 반환하고, 그렇지 않으면 false 반환
	 * 				||(or)		│	두 피연산자 중 하나 또는 모두 true이면 bool true를 반환, 그렇지 않으면 false 반환
