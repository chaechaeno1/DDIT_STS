...
public class HomeController {
	...
	// 해당 경로 : D드라이브 > workspace > .metadata > .plugins > ... > Spring2Project > WEB-INF > views > sub > ... .jsp
	 ...
	 * 
	 * 		@ResponseBody를 지정하지 않으면 HTTP 404에러 발생
	 * 		@ResponseBody가 객체를 리턴하여 객체를 응답 데이터로 보내는 역할을 한다.
	 * 		@ResponseBody의 리턴 defalut 데이터 형식은 json 
	 * 		@ResponseBody대신에 @RestController를 이용하여 대체 가능
	 * 			-> 클래스라인에 @RestController를 붙이게 되면 각각 메서드에 @responseBody 붙이지않아도됨
	 * 
	 */
	
	
	@ResponseBody //return값을 데이터로 인식
	...
	public Member goHome0301() {
		 ...
		 * 이때, jackson-databind 라이브러리가 설정되어 있지 않은 상태에서 요청시, 406 에러 발생
		 * 또, lombok 또는 VO에 getter/setter가 정상적으로 설정되어있지 않은 경우에도 406에러 발생할 수 있음
		 * @ResponseBody를 설정하지 않고 객체를 응답으로 내보낼 시, 데이터로 인식되는게 아닌 페이지로 인식되므로 404에러 발생
		 * 
		 */
	 ...
	 * 
	 * 		- response 할때 HTTP 헤더 정보와 내용을 가공하는 용도로 사용
	 * 
	 */
	...
	// 객체의 JSON 객체 타입의 데이터와 200 OK 상태코드 전송
	@ResponseBody
