...
import kr.or.ddit.service.impl.ItemServiceImpl;
...
public class FileUploadController02 {
	 ...
	 * 	3. 여러개의 이미지 업로드
	 * 
	 * 		- 한 번에 여러개의 이미지를 업로드 하는 파일 업로드 기능을 구현한다.
	 * 		
	 * 		# 데이터 베이스 만들기
	 * 		> item2 테이블을 활용
	 * 
	 ...
	 * 			- 파일 업로드 등록 기능 서비스 인퍼테이스 메소드 만들기
	...
	public String item2Modify(Item2 item, Model model) throws IOException {
		...
		for(int i = 0 ; i < pictures.size(); i++) {
			...
			if(file != null && file.getSize() > 0) {
				...
				String savedName = uploadFile(file.getOriginalFilename(), file.getBytes());
				if(i ==0) item.setPictureUrl(savedName);
				if(i ==1) item.setPictureUrl2(savedName);
	...
	//2개의 파일 출력을 해야하므로 displayFile과 displayFile2 메서드 가 존재해야 한다.
	@ResponseBody
