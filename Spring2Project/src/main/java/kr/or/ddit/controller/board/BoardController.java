...
public class BoardController {
	 ...
	 *	1. 요청 경로 매핑
	 *		
	 *		- @RequestMapping의 value 속성에 요청 경로 설정
	 *		- 요청 경로는 반드시 설정해야 하는 필수 정보
	 *		- 속성이 하나일 때는 속성명 생략 가능 (defalut는 value)
	 *		- 컨트롤러의 클래스 레벨과 메서드 레벨로 지정할 수 있음
	 *		- 클래스 레벨로 요청 경로 지정하면 메서드 레벨에서 지정한 경로의 기본 경로로 취급
	 *		- 클래스 레벨의 요청 경로에 메서드 레벨의 요청 경로를 덧붙인 형태가 최종 경로가 된다.
	 * 
	 */
	
	
	//lombok을 설치하지 않았으므로 logger 직접 설정해주기
	...
	public String registerFormHttp() {
		log.info("registerFormHttp() 실행...! GET방식으로 요청");
		return "success";
	}
	...
	public String registerHttp() {
		log.info("registerFormHttp() 실행...! POST방식으로 요청");
		return "success";		
	}
	...
	public String modifyFormHttp() {
		log.info("modifyFormHttp() 실행...! GET방식으로 요청");
		return "success";
	}	
	...
	public String modifyHttp() {
		log.info("modifyHttp() 실행...! POST방식으로 요청");
		return "success";
	}	
	...
	public String removeHttp() {
		log.info("removeHttp() 실행...! POST방식으로 요청");
		return "success";
	}
	...
	public String listHttp() {
		log.info("listHttp() 실행...! GET방식으로 요청");
		return "success";
	}
	...
	public String registerParamsPost() {
		...
		return "board/list"; //등록 이후에 나가야 하는 페이지 이므로 list로 경로 설정
	}	
	...
	// consumes 속성 값에 "application/json" 미디어 타입을 지정
	...
	// consumes 속성 값에 "application/xml" 미디어 타입을 지정
