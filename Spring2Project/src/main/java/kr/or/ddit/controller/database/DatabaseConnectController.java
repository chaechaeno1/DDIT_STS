...
public class DatabaseConnectController {
	 ...
	 * 			- https://junesker.tistory.com/80 (SqlDeveloper 설치)
	 * 
	 * 		3. 데이터 소스 설정(p.458)
	 ...
	 * 
	 * 
	 * 		5. 스프링 JDBC (p462) (우리는 직접 사용하진 않지만 설명만....)
	 * 
	 ...
	 * 				- 등록, 삭제, 수정, 상세보기 등의 페이지를 구현하며 JDBC 방식 기반의 CRUD를 작성합니다.
	 ...
	 * 
	 * 			- JPA는 자바 표준 ORM입니다.
	 * 				즉, 구현되어 있는 클래스에 테이블을 매핑하기 위한 프레임워크 입니다.
	 * 
	 * 			# 장점
	 ...
	 * 				- EntityManager에서 제공하는 Entity 조작 API를 이용해 Entity에 대해 CRUD작업을 할 수 있다.
	 * 
	 * 			# Entity 상태
	 * 			- new 상태
	 * 			- 관리 상태
	 ...
	 * 			설정된 Entity들을 이용하여 jpa객체를 통해서 함수로 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data API가
	 * 			자동으로 Entity를 분석하여 함수에 따른 쿼리를 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
	 * 
	 * 			기존 SQL 쿼리를 이용하여 서비스를 처리할 때에 필드가 추가되면 필드와 관련되어 있는 기능, SQL, 추가된 필드 등을 수정해야하는
	 * 			번거로움이 발생하지만 JPA는 필드만 추가 후 테이블 정보와 매핑만 시켜주면 JPA가 자동으로 분석을 진행한다.
	 * 			그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들지않고 함수와 같은 기능들로만
	 * 			쿼리를 요청하고 데이터베이스와 통신하기 때문에 데이터베이스 종류엔 크게 상관이 없다.)
	 * 
	 * 			JPA는 JPA 구현체인 hibernate를 이용하여 사용합니다. 그러기 위해선 관련 의존관계를 등록 후 사용해야 합니다.
	 * 			SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 
	 * 			장점이 있어 객체모델을 이용하여 비즈니스 로직을 구성하는데에만 집중을 할 수 있다.
	 * 
	 * 			하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘 못되는 경우 해당 프로젝트의 품질이 떨어진다는 문제가 있습니다.
	 ...
	 * 		7. 마이바티스
	 * 			
	 * 			- SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임워크 입니다.
	 * 
