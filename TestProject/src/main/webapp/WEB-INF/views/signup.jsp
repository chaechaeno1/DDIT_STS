				...
				<div class="card card-plain">
					...
					<div class="card-body">
						<form role="form" method="post" action="/signup.do"
							id="signupForm">
							<font class="font-weight-bold text-xs mt-1 mb-0 error"> <!-- 아이디 에러처리 -->
							</font>
							...
							<font
								class="text-primary font-weight-bold text-xs mt-1 mb-0 error">
								<!-- 비밀번호 에러처리 -->
							</font>
							<div class="input-group input-group-outline mb-3">
							...
							<div class="input-group input-group-outline mb-3">
								<input type="text" class="form-control" id="memPwRe"
							...
							<font class="font-weight-bold text-xs mt-1 mb-0 error"> <!-- 닉네임 에러처리 -->
							</font>
							<div class="input-group input-group-outline mb-3">
								<input type="text" class="form-control" id="memNickname"
									name="memNickname" placeholder="닉네임"> <input
									type="button" class="form-control" id="nickChkBtn" value="중복확인" />
							</div>
							<div class="form-check form-switch">
								<input class="form-check-input" type="checkbox" id="agree"
									value="Y"> <label class="form-check-label" for="agree">개인정보
									동의</label>
							</div>
							<font
								class="text-primary font-weight-bold text-xs mt-1 mb-0 error"></font>
	...
	$(function() {
		...
		memId.on("keyup",function() {
			...
			if (id.length < 4) { //아이디의 길이가 4자리 미만이면..
				errPrint(0, "아이디는 4자리부터 시작합니다!", "red"); //커스텀 메소드 만들어서 사용 
				return false;
			}
		...
		nickChkBtn.on("click", function() {
			...
			$.ajax({
				...
				success : function(res) {
					if (res === "NOTEXIST") {
						errPrint(3, "사용가능한 닉네임 입니다!", "green");
						nickFlag = true;
					} else {
